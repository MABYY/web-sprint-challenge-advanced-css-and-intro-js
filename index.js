...
function get20s(arr){
  ...
  for (i = 0; i < arr.length ;i++) {
    let death_yr =  arr[i].years.split('-')[1];
    if(death_yr>= 1900 && death_yr <=2000) {
      artists_arr.push(arr[i].name);
    }
  }
  return artists_arr; 
}
...
Example: addArtist(artists) should return the artists array with the above
 object added to the end of the array. */

function addArtist(arr){
  let new_obj = { 
    id: 20,
    name: "Jeniffer Williams", 
    years: "1990 - current day",
    genre: "Web Design", 
    nationality: "United States",
    bio: "Switching careers is on"
  };
 let new_artists = arr.push(new_obj);
  return  arr;
  }
...
1. Receive an array 
2. Return an array with names of artists in the received array
 who painted more than 100 paintings.
...
<div class = "bio">Vincent Willem van Gogh (Dutch: [ˈvɪnsɛnt ˈʋɪləm vɑŋ ˈɣɔx] 
  (listen); 30 March 1853 – 29 July 1890) was a Dutch Post-Impressionist painter 
  ...
  around 860 oil paintings, most of them in the last two years of his life. 
  They include landscapes, still lifes, portraits and self-portraits, and are 
...
Create a function called `getHTML()` that takes the parameter `data` and uses a 
for..in loop to format and console.log data like the example above. 

The function should console.log 50 chunks of HTML code that match the structure 
above. 

‼️ You do **NOT** need to get these to display on your page, but you can 
copy and paste the result into your HTML file if you'd like to see what 
...
function getHTML(data){
  ...
  for (var i = 0; i<data.length; i++){
    html_list.push("<div class ='name'><a href=" + data[i].wikipedia + ">" + data[i].name +"</a></div> " + " <div class = 'bio'>"+ data[i].bio + "</div></div>")  ;
  }
  return html_list;
  }
...
/* 💪💪💪💪💪💪 STRETCH 2: 💪💪💪💪💪💪
Create a function called `randomize` that takes a data array as an argument and 
returns a the same array in a randomized order. */
...
console.log(randomize(arr1));


 /* 💪💪💪💪💪💪 STRETCH 3: 💪💪💪💪💪💪
 Use advanced array methods (.map, .reduce, .filer) to refactor your 
 MVP code (create an array of all artists born in the 1900s with .filter, for example) */

function artists_90 (arr){
